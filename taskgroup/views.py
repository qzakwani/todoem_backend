...
def create_taskgroup(req, *args, **kwargs):
    try:
        ...
        if users is None or len(users) > 20 or users is not isinstance(list):
...
def update_taskgroup(req, taskgroup_id,*args, **kwargs):
    try:
        ...
        ser = TaskGroupSerializer(instance=taskgroup, data=req.data)
        if ser.is_valid():
            ser.save()
            return Response(ser.data, status=status.HTTP_201_CREATED)
        return Response(ser.errors, status=status.HTTP_400_BAD_REQUEST)
...
def add_member(req, taskgroup_id, member_id, *args, **kwargs):
    ...
    except NotConnectedLister as err:
